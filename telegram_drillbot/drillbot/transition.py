...
class MenuTransition(Transition):
    ...
    def __init__(self, options, title=None, title_func=None, photo=None, caption=None):
        ...
        self.photo = photo                      # добавил поле для фото
    ...
    def move_to(self, machine):
        ...
        Добавил параметр self.photo в который передается ссылка на фото для его отправки пользователю
        """
        if self.photo:
            print('stransition start send_photo()')
            photo_message = machine.send_keyboard(self.title_func(machine.get_data()), CALL_COMMANDS, self.options, self.photo, self.caption)
            print(f'transition photo_message: {photo_message}')
            # machine.info.photo_message = photo_message
        else:
            print('transition no photo')
            print(f'transition no photo message: {machine.info.photo_message}')
            if machine.info.photo_message:
                machine.delete_photo_message()
